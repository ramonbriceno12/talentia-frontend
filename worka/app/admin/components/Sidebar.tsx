...
export default function Sidebar({ isOpen, closeSidebar }: { isOpen: boolean; closeSidebar: () => void }) {
  ...
  const [notifications, setNotifications] = useState<{ message: string; type: string; sender_id: number }[]>([]);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center mb-6 relative">
        ...
        {/* Notification Bell */}
        <div className="relative mt-3 cursor-pointer" onClick={() => setDropdownOpen(!isDropdownOpen)}>
        ...
        {isDropdownOpen && (
          <div className="absolute top-16 left-0 w-56 bg-[#2a5a65] text-white shadow-lg rounded-md p-3">
            <h4 className="text-center text-sm font-semibold mb-2">Notificaciones</h4>
            {notifications.length === 0 ? (
              <p className="text-gray-400 text-center text-xs">No hay notificaciones</p>
            ) : (
